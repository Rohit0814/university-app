...
@elseif ($boxType=='profile')
    ...
    <div class="flex items-center max-[480px]:w-full">
        ...
        <input type="file" value="{{ $labelValue }}" name="{{ $name }}" class="w-96 remove-name pl-20 disabled:opacity-75 ins-name max-[630px]:w-60 max-[480px]:w-full max-[480px]:pl-3">
        </div>
        {{-- <div class="w-16 h-16 rounded-full" style="background: url('{{ asset(storage/images/.$labelValue) }}'); background-position:center center; background-size:cover"></div> --}}
        @if ($labelValue==NULL)
            <img class="w-16 h-16 rounded-full bg-blue-300" src="https://cdn-icons-png.flaticon.com/512/149/149071.png" >
        @else
